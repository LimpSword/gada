...
def generate_graph_from_json(json_data):
    ...
    def add_nodes(parent_id, children, depth):
        ...
        for child in children:
            ...
            current_id = node_id
            G.add_node(current_id, type=node_type, depth=depth, color = 'skyblue')  # Ajout de l'attribut 'type', 'depth' et color
    ...
    def delete_chain_nodes():
        ...
        while continue_delete:
            ...
            L=[]
            for node in G.nodes:
                if G.nodes[node]['type'] == '' and len(list(G.successors(node))) == 1 and len(list(G.predecessors(node))) == 1 and G.nodes[list(G.successors(node))[0]]['type'] == '' and G.nodes[list(G.successors(node))[0]]['color'] == 'skyblue':
                    L.append(node)
    ...
    node_types = {node: G.nodes[node]['type'] for node in G.nodes}
    #pos = nx.multipartite_layout(G, subset_key='depth', align='horizontal', scale=40)
    nx.drawing.nx_pydot.write_dot(G, "./test/parser/graph.dot")
    ...
    nx.draw_networkx_labels(G,labels=node_types, pos=pos_labels, font_color='black', font_size=8, font_weight='bold')
    #nx.draw(G, pos,labels=node_types, node_size=20, node_color=colors, font_size=9, font_color='black', with_labels=True )
    plt.savefig("./test/parser/graph.png")
    plt.show()

# Chargement des données depuis le fichier JSON
with open('./test/parser/return.json', 'r') as file:
